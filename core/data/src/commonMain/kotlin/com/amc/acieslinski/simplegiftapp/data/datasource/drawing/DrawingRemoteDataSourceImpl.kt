...
class DrawingRemoteDataSourceImpl(
    ...
    @Throws(RequestException::class, CancellationException::class)
    override suspend fun saveDrawing(request: SaveDrawingRequestModel) {
        saveDrawing(request.drawing, request.privateToken)
    }

    private suspend fun saveDrawing(drawing: DrawingRemoteModel, privateToken: String) {
        httpClient
